...
from database.models import Community, CommunityMember, CommunityPlace, CommunityCheckinJob, User
        ...
        for community in communities:
            ...
            if member_user_ids:
                ...
                for member in members:
                    member.user = next((u for u in users if u.id == member.user_id), None)
...
async def create_community_place(self, info: Info, input: CommunityPlaceInput) -> CommunityType:
    ...
    with Session(engine) as session:
        ...
        member_query = select(CommunityMember).where(
            CommunityMember.community_id == input.communityId,
