...
from graphql_utils.walk import (
    CreateWalkInput,
    WalkDayActivity,
    WalkIntervalActivity,
    WalkIntervalActivityItem,
    WalkType,
)


async def get_walks_resolver(
    self,
    info: Info,
...
async def get_walk_resolver(self, info: Info, id: str) -> WalkType | None:
    user = await check_authentication(info)
    if not user.id:
        raise Exception("User not found")
...
async def create_walk_resolver(self, info: Info, input: CreateWalkInput) -> WalkType:
    ...
    walk = Walk(started_at=input.startedAt, finished_at=input.finishedAt, user_id=user.id)
...
async def get_walk_interval_activity_by_day(
    ...
    query = select(Walk).where(
        Walk.user_id == user.id, Walk.started_at >= from_date, Walk.started_at < to_date
    )

    walk_interval_activity = WalkIntervalActivity(totalDistance=0.0, totalDuration=0.0, items=[])

    walk_interval_activity_date_map: dict[datetime, float] = {}

    for i in range((to_date - from_date).days):
        walk_interval_activity_date_map[(from_date + timedelta(days=i))] = 0.0
    ...
    with Session(engine) as session:
        ...
        for key, value in walk_interval_activity_date_map.items():
            walk_interval_activity.items.append(
                WalkIntervalActivityItem(
                    duration=value,
                    date=datetime.fromtimestamp(key.timestamp(), tz=from_date.tzinfo),
                )
            )
