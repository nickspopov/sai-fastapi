...
from graphql_utils.community import CommunityMemberType, CommunityPlaceType, CommunityType
...
class UserDog(SQLModel, table=True):
    ...
    user_id: str | None = Field(foreign_key="user.id", primary_key=True)
    dog_id: str | None = Field(foreign_key="dog.id", primary_key=True)
...
class CommunityPlace(SQLModel, table=True):
    ...
    def to_graphQL(self):
        return CommunityPlaceType(
            id=str(self.id), name=self.name, lat=self.latitude, lon=self.longitude
        )
...
class WalkInterval(SQLModel, table=True):
    ...
    def to_graphQL(self):
        return WalkHistoryItemType(
            latitude=self.latitude, longitude=self.longitude, timestamp=self.timestamp
        )
...
class Walk(UUID, table=True):
    ...
    def get_distance(self) -> float:
        ...
        for i in range(len(self.intervals) - 1):
            distance += self.get_distance_between_two_points(
                self.intervals[i].latitude,
                self.intervals[i].longitude,
                self.intervals[i + 1].latitude,
                self.intervals[i + 1].longitude,
            )
        return distance
    ...
    def to_graphQL(self):
        walk_history = WalkHistoryType(
            history=[interval.to_graphQL() for interval in self.intervals]
        )
        return WalkType(
            id=str(self.id),
